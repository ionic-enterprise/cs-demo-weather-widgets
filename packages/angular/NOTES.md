...
The Angular Framework Wrapper is configured in a similar manner to the other output targets. See the [framework wrapper documentation](https://github.com/ionic-team/stencil-ds-output-targets/blob/main/packages/angular-output-target/README.md) for a full set of options.
...
This approach will use the `dist` output from your Stencil component library. Update your `stencil.config.ts`:
...
The advantage with SCAMs is it makes it easier for consumers of the library to control their bundle sizes. The disadvantage is a slightly higher maintenance cost when it comes to maintaining the component library. Since this cost is _only_ incurred if a component is added, removed, or renamed, the cost should not be too high.

The single component angular module uses the `dist-custom-elements` output from your Stencil component library. Update your `stencil.config.ts`:
...
The standalone components use the `dist-custom-elements` output from your Stencil component library. Update your `stencil.config.ts`:
...
export * from './lib/cs-demo-weather-widgets.module'; // Only needed if using the one-module approach
```
